...
7. **Dungeon:** add a Browser Source pointed at `https://<your-app>/dungeon` (locally `http://localhost:3000/dungeon`). Use a small 16:9 size in OBS: **480×270**, **640×360**, **854×480**, or **960×540**. Chat moves a shared first-person maze with `!up` `!down` `!left` `!right`. Starts in anarchy; mods can switch with `!anarchy` / `!democracy`. `!dungeon` resets to floor 1. Test walk: `/dungeon?preview=1`.
